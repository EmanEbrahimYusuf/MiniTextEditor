...
void Text_Editor::Add_Line(string text)
{
	...
	count++;
	cout << endl << "Add Line Operation Done Successfully...!\n";
	output.close();
}
...
void Text_Editor::Insert_Line(int index, string text)
{
	...
	else
	{
		...
		count++;
		cout << endl << "Insert Line Operation Done Successfully...!\n";
...
string Text_Editor::Get_Line(int index)
{
	...
	if (IsEmpty())
	{
		cout << endl << "Get Line is Invaild, This Text Editor is Empty\n";
		return temp->data;
	}
	else if (index >= count)
	{
		cout << endl << "Get Line is Invaild, The Index You Entered Not Found\n";
		return temp->data;
	}
	else
	{
...
void Text_Editor::Delete_Line(int index)
{
	...
	node* temp, * ptr;
	if (IsEmpty())
		cout << endl << "Delete Line is Invaild, This Text Editor Already is Empty\n";
	else if (index >= count)
		cout << endl << "Delete Line is Invaild, The Index You Entered Not Found\n";
	else if (index == 0)
	{
		node* a = head;
		head = head->next;
	...
	else
	{
		...
		ptr->next = temp->next;
		delete temp;
		cout << endl << "Delete Line Operation Done Successfully...!\n";
		count--;
		ofstream output("default.txt");
		node* a = head;
		for (int i = 0; i < count; i++)
...
void Text_Editor::Update_Line(int index, string text)
{
	index -= 1;
	if (IsEmpty())
		cout << endl << "Update Line is Invaild, This Text Editor is Empty\n";
	else if (index >= count)
		cout << endl << "Delete Line is Invaild, The Index You Entered Not Found\n";
	else
	{
		...
		temp->data = text;
		cout << endl << "Update Line Operation Done Successfully...!\n";
...
void  Text_Editor::FindAll(string text)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		while ((index = temp->data.find(text, index)) != string::npos)
		{
			char before = temp->data[temp->data.find(text) - 1];
			char after = temp->data[temp->data.find(text) + text.length()];
			cout << "befoooooore" << before << endl;
			cout << "line " << temp->data << endl;
			cout << "afteeeeeer" << after << endl;
			cout << "pppppppppppppppppppppppppppppppp" << temp->data.find(text) << endl;
			index += text.length();
			if (before == comma[0] && after == ' ')
			{
				cout << "000000000000000The line is: \"" << temp->data << "\"\n";
				check = true;
				c++;
			}
			else if ((temp->data.find(text) == 0) && before == temp->data[temp->data.length() - 1])
			{
				cout << "1111111111111111The line is: \"" << temp->data << "\"\n";
				check = true;
				c++;
			}
			else if (((temp->data.find(text) == 0) && (temp->data[temp->data.find(text) + text.length()] == ' ')))
			{
				cout << "222222222222The line is: \"" << temp->data << "\"\n";
				check = true;
				c++;
			}
			else if (before == ' ' && after == ' ')
			{
				cout << "333333333333The line is: \"" << temp->data << "\"\n";
				check = true;
				c++;
			}

			else if (before == ' ' && temp->data[temp->data.find(text) + text.length() - 1] == before)
			{
				cout << "44444444444The line is: \"" << temp->data << "\"\n";
				check = true;
				c++;
			}
	...
	if (!check)
	{
		cout << " There's Nothing matches here..." << endl;
	}
	else
	{
...
void Text_Editor::Find_ReplaceAll(string t1, string t2)
{
	...
	cout << "Replace Texts That Contains This Text \"" << t2 << "\" to \"" << t1 << "\" :" << endl;
	for (int i = 0; i < count; i++)
	{
